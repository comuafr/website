---
name: "Pédale pour tour"
price: 79
category: "accessoires"
image: "/images/products/pedale.jpg"
description: "Pédale de commande pour tour de potier. Contrôle progressif de la vitesse de rotation."
specs:
  Type: "Pédale de commande"
  Fonction: "Contrôle de vitesse"
  Compatibilité: "Tours COMUA"
inStock: true
order: 4
---

La **Pédale pour tour** permet un contrôle progressif et précis de la vitesse de rotation de votre tour de potier. Conçue pour un confort d'utilisation optimal au pied.
